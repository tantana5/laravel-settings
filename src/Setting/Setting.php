...
class Setting
{
    ...
    {
        if (strpos($key, '.') !== false) {
            $this->setSubValue($key, $value, $collection);
        } else {
            $this->setByKey($key, $value, $collection);
        }
    ...
    {
        return $this->storage->where('collection', $collection)->get();
    }
    ...
    protected function setByKey($key, $value, $collection = '')
    {
        if (is_array($value)) {
        ...
        if ($this->hasByKey($main_key)) {
            ...
            $this->storage->store($main_key, $value, $this->lang, $collection);
        }
    ...
    protected function setSubValue($key, $new_value, $collection)
    {
        $setting = $this->getByKey($key);
        ...
        $this->setByKey($key, $setting, $collection);
    }
